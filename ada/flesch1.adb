...
procedure flesch1 is

    In_File                                          : File_Type;
    value, char                                      : Character;
    string_array, words                              : array(1..5000000) of Character;
    pos, sc, wc, nd, vc, count, wvc, start, endpos   : Integer;
    a, b, index, grade, scf, wcf, vcf                : Float;
    word                                             : array(1..30) of Character; 
    vp                                               : Boolean;
...
--count(counter for each word), wvc(word vowel count), start(start of a word), endpos(end of a word), vp(previous vowel)
    wvc:=0;
    vc:=0;
    start:=1;
    for x in 1..nd loop
        if words(x) = ' ' then
            endpos:=x;
            --go through string at space start -> endpos and puts each word in word
            count:=1;
            for y in start..endpos loop
                word(count):=words(y);
                count:=count+1;
            end loop;
            --goes through each word counting for vowels
            count:=endpos-start;
            for z in 1..count loop
                if is_Vowel(word(z)) then
                    wvc:=wvc+1;
                    vp:=true;
                else
                    vp:=false;
                end if;
                if is_Vowel(word(z+1)) and vp = true then
                    wvc:=wvc-1;
                end if;
                if word(z) = 'e' and word(z+1) = ' ' and wvc>=2 then
                    wvc:=wvc-1;
                end if;
            end loop;
            vc:=vc+wvc;
